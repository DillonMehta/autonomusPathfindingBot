...
//For every exit command in every fwd function, step must increment because the motion is inside of the loop
...
void fwd(int intent) {
  ...
  while (!(eCount / (CLICKS_PER_ROTATION * GEAR_RATIO) * WHEEL_CIRCUMFERENCE >= intent)) {
    ...
    countL();
    countR();
    motors.setSpeeds( (intent - d1()) * Kpf + s1min, (intent - d1()) * Kpf + s2min - ang() * Kps);
