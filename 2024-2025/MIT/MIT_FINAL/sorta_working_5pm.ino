...
const float BOT_RADIUS = 4.35; //cm horizontal radius(wheels to center)
...
double Kpt         = 1.5;    // proportional factor for turning
...
void fwd(double distance) {
  ...
  double delta_T = findTime(distance) * 0.90;
...
void back(double distance) {
  ...
  update();
  double t0 = micros();
  double delta_T= findTime(distance) + 1;             // Start time in microseconds
  double delta_T_us = delta_T * 1e6; // Convert delta_T from seconds to microseconds
  ...
  while (true) {
    ...
    left_pwm += kP * velocity_error_L + kPs * ang();
    right_pwm += kP * velocity_error_R;
