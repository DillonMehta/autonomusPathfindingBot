...
//turning
float pwm_min = 27;
float Kpt = 0.5; //for turning
float left_angle=90;
float right_angle=90;
float kPs = 0.2; //to go straight, only affects right motor
float kP = 0.4; 
const float turnTime = 1500; //time for each turn in ms.
//Movement Values (Change here) ------------------------------------------------------------------------------------------------------------------------

float targetTime = 60;
float end_distance = 50;
float end_delay = 1000;
...
void fwd(double distance) {
  ...
  while (true) {
    ...
    right_pwm += kP * velocity_error_R -  kPs * (ang() - angle_pointer);
...
void end(double d) {
  ...
  while (true) {
    ...
    // Update PWM values based on velocity feedback and setpoint
    double velocity_error_L = velocity_setpoint - vL();
    double velocity_error_R = velocity_setpoint - vR() -  kPs * (ang() - angle_pointer);
