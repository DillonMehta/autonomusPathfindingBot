...
double kPs  = 0; // to go straight, only affects right motor
double kP   = 2;
const double turnTime = 1600; // time for each turn in ms.
float accel_time = 3e5; //0.4s

//Movement Values (Change here) ------------------------------------------------------------------------------------------------------------------------
double targetTime    = 2;
double end_distance  =  50;//44.5 + 50 - BOT_RADIUS;
double end_delay     = 0;
char movement[200] = "F58.6";
//"F34.8 R F50 L F100 R F50 L F50 B50 L F200 L F100 L F50 L F150 R F100 R F150 L F50 L F50 B50 L F50 R F150 L E";
...
void fwd(double distance) {
  ...
  while (true) {
    ...
    if ((dL() + dR())/2 >= 0.7 * distance) {
      break;
    }
    
    // Accel / constant / decel phases
    if (elapsed_time <= accel_time) {
      velocity_setpoint = t_vel/accel_time * (elapsed_time);
    } else {
      velocity_setpoint = t_vel;
    }
    ...
    motors.setSpeeds(str_min + left_pwm, str_min+ right_pwm);
  }
  while ((dL() + dR())/2 <= distance){
    update();
    motors.setSpeeds(str_min + left_pwm*(1-(dL()-0.7*distance)/0.3/distance ), str_min + right_pwm*(1-(dL()-0.7*distance)/0.3/distance ) - kPs * ang());
  }
...
void findVel(){t_vel = total_distance/(targetTime - total_turns*turnTime/1e3 - t_str * accel_time/1e6);}
