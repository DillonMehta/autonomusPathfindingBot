...
char movement[200] = "F150 L F20 L F150 R B50 R";
...
void curveTurn(bool dir){
  ...
  while (true)
  {
    ...
    if(dir){
      velocity_error_L = velocity_setpoint - vL();
      velocity_error_R = 3/2 * velocity_setpoint - vR();
    }else{
      velocity_error_L = 3/2* velocity_setpoint - vL();
      velocity_error_R = velocity_setpoint - vR();
    }
    ...
    right_pwm = constrain(right_pwm, str_min, 400);
    if(dir){
      right_pwm -= swerve_Kpt * (vL()/vR() - 2/3);
    }else{
      left_pwm -= swerve_Kpt * (vR()/vL() - 2/3);
    }
    // Set motor speeds
    motors.setSpeeds(left_pwm, right_pwm);
  }
