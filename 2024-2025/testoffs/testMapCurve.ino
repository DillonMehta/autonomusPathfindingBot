...
double targetTime = 73; // subtract 1.75 seconds for every 50cm the end has, including the dist_diff
// 20 seconds for turn cali codes
double end_distance = 19.5; //
double end_delay = 0;
...
void curveTurn(bool dir){
  ...
  if(dir){
    left_pwm = pwm_min;
    right_pwm = pwm_min*1/ratio;
  }
  else{
    right_pwm = pwm_min;
    left_pwm = pwm_min*1/ratio;
  }
  ...
  // Stop motors at the end
  motors.setSpeeds(0, 0);
  delay(110 + delayer_amt); // Small delay to ensure stop
  reset();                 // Reset necessary parameters
